...
void ProxyRTSPClient::continueAfterSETUP(int resultCode) {
  ...
  if (fVerbosityLevel > 0) {
    envir() << *this << "::continueAfterSETUP(): head codecProxy: " << fSetupQueueHead->codecName()
...
FramedSource* ProxyServerMediaSubsession::createNewStreamSource(unsigned clientSessionId, unsigned& estBitrate) {
  ...
  if (fClientMediaSubsession.readSource() == NULL) {
    ...
    if (fClientMediaSubsession.readSource() != NULL) {
      // First, check whether we have defined a 'transcoder' filter to be used with this codecProxy:
...
::createNewRTPSink(Groupsock* rtpGroupsock, unsigned char rtpPayloadTypeIfDynamic, FramedSource* inputSource) {
  ...
  // Create (and return) the appropriate "RTPSink" object for our codecProxy:
  ...
  if (strcmp(fCodecName, "AC3") == 0 || strcmp(fCodecName, "EAC3") == 0) {
    ...
    // This codecProxy requires a specialized RTP payload format; however, we don't yet have an appropriate "RTPSink" subclass for it:
    if (verbosityLevel() > 0) {
      envir() << "\treturns NULL (because we don't have a \"RTPSink\" subclass for this RTP payload format)\n";
    }
    return NULL;
  } else {
    // This codecProxy is assumed to have a simple RTP payload format that can be implemented just with a "SimpleRTPSink":
