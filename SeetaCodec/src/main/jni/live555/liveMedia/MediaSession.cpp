...
Boolean MediaSession::initializeWithSDP(char const* sdpDescription) {
  ...
  while (sdpLine != NULL) {
    ...
    // If we don't yet know the codecProxy name, try looking it up from the
	...
	envir().setResultMsg("Unknown codecProxy name for RTP payload type ",
			     typeStr);
	return False;
      }
    }
...
char* MediaSession::lookupPayloadFormat(unsigned char rtpPayloadType,
  ...
  // Look up the codecProxy name and timestamp frequency for known (static)
  // RTP payload formats.
...
Boolean MediaSubsession::parseSDPAttribute_rtpmap(char const* sdpLine) {
  // Check for a "a=rtpmap:<fmt> <codecProxy>/<freq>" line:
  ...
  if (sscanf(sdpLine, "a=rtpmap: %u %[^/]/%u/%u",
    ...
    if (rtpmapPayloadFormat == fRTPPayloadFormat) {
      ...
      // codecProxy name and timestamp frequency:
      // (First, make sure the codecProxy name is upper case)
      {
	Locale l("POSIX");
