...
unsigned
RTSPClient::sendOptionsCommand(responseHandler *responseHandler, Authenticator *authenticator) {
...
unsigned
RTSPClient::sendAnnounceCommand(char const *sdpDescription, responseHandler *responseHandler,
                                Authenticator *authenticator) {
    if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;
    return sendRequest(
            new RequestRecord(++fCSeq, "ANNOUNCE", responseHandler, NULL, NULL, False, 0.0, 0.0,
                              0.0, sdpDescription));
}
...
unsigned RTSPClient::sendPlayCommand(MediaSession &session, responseHandler *responseHandler,
    ...
    return sendRequest(
            new RequestRecord(++fCSeq, "PLAY", responseHandler, &session, NULL, 0, start, end,
                              scale));
}
...
unsigned RTSPClient::sendPlayCommand(MediaSubsession &subsession, responseHandler *responseHandler,
    ...
    return sendRequest(
            new RequestRecord(++fCSeq, "PLAY", responseHandler, NULL, &subsession, 0, start, end,
                              scale));
}
...
unsigned RTSPClient::sendPlayCommand(MediaSession &session, responseHandler *responseHandler,
    ...
    sendDummyUDPPackets(session); // hack to improve NAT traversal
    return sendRequest(
            new RequestRecord(++fCSeq, responseHandler, absStartTime, absEndTime, scale, &session,
                              NULL));
}
...
unsigned RTSPClient::sendPlayCommand(MediaSubsession &subsession, responseHandler *responseHandler,
    ...
    sendDummyUDPPackets(subsession); // hack to improve NAT traversal
    return sendRequest(
            new RequestRecord(++fCSeq, responseHandler, absStartTime, absEndTime, scale, NULL,
                              &subsession));
}
...
unsigned
RTSPClient::sendRecordCommand(MediaSubsession &subsession, responseHandler *responseHandler,
                              Authenticator *authenticator) {
    if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;
...
unsigned
RTSPClient::sendTeardownCommand(MediaSubsession &subsession, responseHandler *responseHandler,
                                Authenticator *authenticator) {
    if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;
...
unsigned
RTSPClient::sendSetParameterCommand(MediaSession &session, responseHandler *responseHandler,
    ...
    unsigned result = sendRequest(
            new RequestRecord(++fCSeq, "SET_PARAMETER", responseHandler, &session, NULL, False, 0.0,
                              0.0, 0.0, paramString));
    delete[] paramString;
    return result;
}
...
unsigned
RTSPClient::sendGetParameterCommand(MediaSession &session, responseHandler *responseHandler,
    ...
    unsigned result = sendRequest(
            new RequestRecord(++fCSeq, "GET_PARAMETER", responseHandler, &session, NULL, False, 0.0,
                              0.0, 0.0, paramString));
    delete[] paramString;
    return result;
}
...
RTSPClient::RTSPClient(UsageEnvironment &env, char const *rtspURL,
    ...
    unsigned userAgentNameSize
            =
            strlen(applicationName) + strlen(libPrefix) + strlen(libName) + strlen(libVersionStr) +
            strlen(libSuffix) + 1;
    char *userAgentName = new char[userAgentNameSize];
    sprintf(userAgentName, "%s%s%s%s%s", applicationName, libPrefix, libName, libVersionStr,
            libSuffix);
...
static char *
createRangeString(double start, double end, char const *absStartTime, char const *absEndTime) {
    char buf[100];
...
Boolean RTSPClient::setRequestFields(RequestRecord *request,
    ...
    if (strcmp(request->commandName(), "DESCRIBE") == 0) {
        ...
        if (strcmp(request->commandName(), "PLAY") == 0) {
            ...
            float speed = request->session() != NULL ? request->session()->speed()
            ...
            extraHeaders = new char[strlen(sessionStr) + strlen(scaleStr) + strlen(speedStr) +
                                    strlen(rangeStr) + 1];
            extraHeadersWereAllocated = True;
...
int RTSPClient::openConnection() {
    do {
        ...
        ignoreSigPipeOnSocket(
                fInputSocketNum); // so that servers on the same host that get killed don't also kill us
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum) {
    ...
    if (fVerbosityLevel >= 1) {
        envir() << "Connecting to " << AddressString(remoteName).val() << ", port " << remotePortNum
                << " on socket " << socketNum << "...\n";
    }
...
Boolean
RTSPClient::checkForHeader(char const *line, char const *headerName, unsigned headerNameLength,
    ...
    if (line[paramIndex] == '\0')
        return False; // the header is assumed to be bad if it has no parameters
    ...
    while (sscanf(fields, "%[^;]", field) == 1) {
        if (sscanf(field, "server_port=%hu", &serverPortNum) == 1) {
            ...
            foundDestinationStr = strDup(field + 12);
                ...
                sscanf(field, "port=%hu-%hu", &multicastPortNumRTP, &multicastPortNumRTCP) == 2 ||
...
Boolean
RTSPClient::parseRTPInfoParams(char const *&paramsStr, u_int16_t &seqNum, u_int32_t &timestamp) {
...
Boolean RTSPClient::handleSETUPResponse(MediaSubsession &subsession, char const *sessionParamsStr,
    ...
    do {
        ...
        if (!parseTransportParams(transportParamsStr, serverAddressStr, serverPortNum, rtpChannelId,
                                  rtcpChannelId)) {
        ...
        if (streamUsingTCP) {
            ...
            RTPInterface::setServerRequestAlternativeByteHandler(envir(), fInputSocketNum,
...
Boolean
RTSPClient::handleTEARDOWNResponse(MediaSession & /*session*/, MediaSubsession & /*subsession*/) {
...
Boolean RTSPClient::handleAuthenticationFailure(char const *paramsStr) {
    ...
    if (sscanf(paramsStr, "Digest realm=\"%[^\"]\", nonce=\"%[^\"]\", stale=%[a-zA-Z]", realm,
               nonce, stale) == 3) {
    ...
    if (success) {
        if ((!realmHasChanged && !isStale) || fCurrentAuthenticator.username() == NULL ||
...
void RTSPClient::responseHandlerForHTTP_GET(RTSPClient *rtspClient, int responseCode,
                                            char *responseString) {
...
void RTSPClient::responseHandlerForHTTP_GET1(int responseCode, char *responseString) {
    ...
    do {
        ...
        ignoreSigPipeOnSocket(
                fOutputSocketNum); // so that servers on the same host that killed don't also kill us
        ...
        do {
            ...
            do {
                ...
                nextLineStart = getLine(lineStart);
                     ...
                     nextLineStart != NULL); // skip over any blank lines at the start
            ...
            while (1) {
                ...
                if (checkForHeader(lineStart, "CSeq:", 5, headerParamsStr)) {
                    ...
                    while ((request = fRequestsAwaitingResponse.dequeue()) != NULL) {
                        if (request->cseq() <
                            cseq) { // assumes that the CSeq counter will never wrap around
                            ...
                            if (fVerbosityLevel >= 1 &&
                                ...
                                envir() << "WARNING: The server did not respond to our \""
                                        << request->commandName() << "\" request (CSeq: "
                                        << request->cseq()
...
// so that other projects that use the code could benefit (not just your own project).
                    ...
                    if (oldBaseURLTail != NULL) {
                        unsigned newBaseURLLen
                                = (oldBaseURLTail - fBaseURL) + 8/* for "/stream=" */ +
            ...
            unsigned bodyOffset =
                    nextLineStart == NULL ? fResponseBytesAlreadySeen : nextLineStart -
                                                                        headerDataCopy;
            ...
            if (contentLength > numBodyBytes) {
                ...
                if (fVerbosityLevel >= 1) {
                    envir() << "Have received " << fResponseBytesAlreadySeen << " total bytes of a "
                            ...
                            << " RTSP response; awaiting " << numExtraBytesNeeded
                            << " bytes more.\n";
                }
                delete[] headerDataCopy;
                if (foundRequest != NULL)
                    fRequestsAwaitingResponse.putAtHead(
                            foundRequest);// put our request record back; we need it again
            ...
            if (foundRequest != NULL) {
                ...
                if (responseCode == 200) {
                    ...
                    if (strcmp(foundRequest->commandName(), "SETUP") == 0) {
                        if (!handleSETUPResponse(*foundRequest->subsession(), sessionParamsStr,
                                                 transportParamsStr,
                                                 foundRequest->booleanFlags() & 0x1))
                            break;
                    } else if (strcmp(foundRequest->commandName(), "PLAY") == 0) {
                        if (!handlePLAYResponse(foundRequest->session(), foundRequest->subsession(),
                                                scaleParamsStr, speedParamsStr, rangeParamsStr,
                                                rtpInfoParamsStr))
                            break;
...
RTSPClient::RequestRecord::RequestRecord(unsigned cseq, char const *commandName,
        ...
        : fNext(NULL), fCSeq(cseq), fCommandName(commandName), fSession(session),
          fSubsession(subsession), fBooleanFlags(booleanFlags),
          fStart(start), fEnd(end), fAbsStartTime(NULL), fAbsEndTime(NULL), fScale(scale),
          fContentStr(strDup(contentStr)), fHandler(handler) {
}
...
RTSPClient::RequestRecord::RequestRecord(unsigned cseq, responseHandler *handler,
                                         ...
                                         float scale,
                                         MediaSession *session, MediaSubsession *subsession)
        : fNext(NULL), fCSeq(cseq), fCommandName("PLAY"), fSession(session),
          fSubsession(subsession), fBooleanFlags(0),
...
::createNew(UsageEnvironment &env, onRTSPClientCreationFunc *creationFunc, Port ourPort,
    ...
    return new HandlerServerForREGISTERCommand(env, creationFunc, ourSocket, ourPort, authDatabase,
                                               verbosityLevel, applicationName);
}

HandlerServerForREGISTERCommand
::HandlerServerForREGISTERCommand(UsageEnvironment &env, onRTSPClientCreationFunc *creationFunc,
                                  int ourSocket, Port ourPort,
                                  UserAuthenticationDatabase *authDatabase, int verbosityLevel,
                                  char const *applicationName)
        : RTSPServer(env, ourSocket, ourPort, authDatabase, 30/*small reclamationTestSeconds*/),
          fCreationFunc(creationFunc), fVerbosityLevel(verbosityLevel),
...
::createNewRTSPClient(char const *rtspURL, int verbosityLevel, char const *applicationName,
                      int socketNumToServer) {
    // Default implementation: create a basic "RTSPClient":
    return RTSPClient::createNew(envir(), rtspURL, verbosityLevel, applicationName, 0,
                                 socketNumToServer);
}
...
::implementCmd_REGISTER(char const *cmd/*"REGISTER" or "DEREGISTER"*/,
    ...
    if (strcmp(cmd, "REGISTER") == 0) { // By default, we don't implement "DEREGISTER"
        ...
        RTSPClient *newRTSPClient = createNewRTSPClient(url, fVerbosityLevel, fApplicationName,
