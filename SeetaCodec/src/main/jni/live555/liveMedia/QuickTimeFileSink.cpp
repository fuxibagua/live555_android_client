...
Boolean SubsessionIOState::setQTstate() {
  ...
  char const* noCodecWarning3 = "\" track, so we'll insert a dummy \"????\" Media Data Atom instead.  A separate, codecProxy-specific editing pass will be needed before this track can be played.\n";
  ...
  do {
	...
	= &QuickTimeFileSink::addAtom_soundMediaGeneral; // by default
      ...
      // Make sure that our subsession's codecProxy is one that we can handle:
    ...
    envir() << "Warning: We don't have sufficient codecProxy-specific information (e.g., sample sizes) to fully generate the \""
	    << fOurSubsession.mediumName() << "/" << fOurSubsession.codecName()
	    << "\" track, so we'll disable this track in the movie.  A separate, codecProxy-specific editing pass will be needed before this track can be played\n";
    fQTEnableTrack = False; // disable this track in the movie
#endif
